...
def send_file(server_ip, client_port, server_port, fragment_size, file_path):
    ...
    while True:
        client_socket.sendto(                                                           # initialization connection
            protocol.msg_initialization(fragment_size, file_size, file_name),           # send info about file
            (server_ip, server_port))                                                   # server address
        data, server_address = client_socket.recvfrom(fragment_size)
        if data.decode('utf-8') == protocol.MsgType.ACK.value:
            break
...
def send_message(server_ip, client_port, server_port, fragment_size, message):
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    client_socket.bind(('', client_port))

    while True:
        client_socket.sendto(                                                                # initialization connection
            protocol.msg_initialization(fragment_size, '', protocol.MsgType.SET_MSG.value),     # send info about msg
            (server_ip, server_port))                                                           # server address
        data, server_address = client_socket.recvfrom(fragment_size)
        if data.decode('utf-8') == protocol.MsgType.ACK.value:
            break
...
def user_interface():
    ...
    while True:
        ...
        if user_input == '0':
            ...
            send_message(server_ip, client_port, server_port, fragmentation, message)
        elif user_input == '2':
            ...
            if not os.path.isfile(file):                                          # check if given path is valid
                ...
                continue
            send_file(server_ip, client_port, server_port, fragmentation, file)
        elif user_input == '9':
